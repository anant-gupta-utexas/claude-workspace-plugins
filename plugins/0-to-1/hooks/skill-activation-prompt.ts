...
function loadSkillRules(): SkillRules {
    ...
    if (existsSync(userRulesPath)) {
        try {
            const userRules: SkillRules = JSON.parse(readFileSync(userRulesPath, 'utf-8'));
            Object.assign(mergedSkills, userRules.skills);
        } catch (err) {
            console.error(`Warning: Failed to load user-level skill-rules.json: ${err}`);
        }
    }

    // Try to load project-level skill rules (project takes precedence)
...
async function main() {
    try {
        ...
        // Load skill rules from both user and project directories
        const rules = loadSkillRules();

        // If no skills loaded, exit silently
        if (Object.keys(rules.skills).length === 0) {
            process.exit(0);
        }
